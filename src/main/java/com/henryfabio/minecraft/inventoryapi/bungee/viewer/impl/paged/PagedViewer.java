package com.henryfabio.minecraft.inventoryapi.bungee.viewer.impl.paged;
...
import com.henryfabio.minecraft.inventoryapi.bungee.item.enums.DefaultItem;
...
//            InventoryItem emptyInventoryItem = InventoryItem.of(new ItemStack(ItemType.AIR));
//            editor.fillCenter(emptyInventoryItem, border); Minecraft Crash!
            ...
            for (int i = 0; i < configuration.itemPageLimit(); i++, pageIndex++) {
                if (pageIndex < pageMaxIndex) {
                    ...
                    inventoryItems.add(DefaultItem.NULL_ITEM.toInventoryItem()); // Setar com NULL ou AIR dá crash no client
                }
            }
            ...
            editor.setItem(
                    ...
                    this.hasNextPage() ? DefaultItem.NEXT_PAGE.toInventoryItem(this) : DefaultItem.NULL_PAGE.toInventoryItem()
            );
            editor.setItem(
                    configuration.previousPageSlot(),
                    this.hasPreviousPage() ? DefaultItem.PREVIOUS_PAGE.toInventoryItem(this) : DefaultItem.NULL_PAGE.toInventoryItem()
            );
        }
    }
    ...
    public void resetConfigurations() {
        ...
        int inventoryLines = configuration.inventoryType().getTypicalSize(47) / 9;

        configuration.border(Border.of(1));

        int emptyPageSlot = configuration.inventoryType().getTypicalSize(47);
